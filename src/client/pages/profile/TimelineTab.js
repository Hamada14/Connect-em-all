  ...
  getAllPosts() {
    return fetch('/api/get_posts_by_user?userId=' + this.props.profileId,
      ...
      .then(posts_data => {
        ...
        return posts.map((post) => { return { profileId: this.props.profileId, writer: post.FULL_NAME, content: post.CONTENT, date: post.CREATED_AT, email: post.EMAIL, likesCount: post.LIKES_COUNT, likers: post.LIKERS, likes: post.LIKES, postId: post.POST_ID, userId: post.USER_ID }} );
      });
  }
